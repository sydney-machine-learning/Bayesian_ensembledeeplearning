...
from torch import device, from_numpy, optim, tensor
...
class Network(nn.Module):
    ...
    def ForwardPass(self, X):
        self.out = self.net(tensor(X, dtype=torch.float32).cuda())
    ...
    def decode(self, w):
        ...
        for name in sorted(self.state_dict().keys()):
            dic[name] = torch.FloatTensor(
                w[i : i + (self.state_dict()[name]).view(-1).shape[0]]
            ).view(self.state_dict()[name].shape)
...
class ptReplica(multiprocessing.Process):
    ...
    def run(self):
        ...
        step_w = 0.025
        # Declare FNN
        fnn = Network(self.topology, self.traindata, self.testdata, learn_rate).cuda()
        # Evaluate Proposals
        ...
        likeh_list = np.zeros(
            (samples, 2)
        )  # one for posterior of likelihood and the other for all proposed likelihood
        ...
        for i in range(
            ...
            if (self.use_langevin_gradients is True) and (lx < self.l_prob):
                ...
                w_prop_gd = fnn.langevin_gradient(
            ...
            [likelihood_proposal, pred_train, rmsetrain] = self.likelihood_func(
                fnn, self.traindata, w_proposal
            )

            [likelihood_ignore, pred_test, rmsetest] = self.likelihood_func(
                fnn, self.testdata, w_proposal
            )
            ...
            prior_prop = self.prior_likelihood(
                sigma_squared, nu_1, nu_2, w_proposal
            )  # takes care of the gradients
            ...
            if u < mh_prob:
                ...
                acc_train[i + 1,] = acc_train[
                    i,
                ]
                acc_test[i + 1,] = acc_test[
                    i,
                ]
...
class ParallelTempering:
    def __init__(
        ...
        self.topology = topology
        self.num_param = (
            (topology[0] * topology[1]) + (topology[1] * topology[2]) + topology[1] + topology[2]
        )
    ...
    def show_results(self):
        ...
        likelihood_rep = np.zeros(
            (self.num_chains, self.NumSamples - burnin, 2)
        ...
        fx_train_all = np.zeros(
            (self.num_chains, self.NumSamples - burnin, self.traindata.shape[0])
        )
        ...
        for i in range(self.num_chains):
            file_name = (
                self.path + "/posterior/pos_w/" + "chain_" + str(self.temperatures[i]) + ".txt"
            )
            ...
            file_name = (
                self.path + "/predictions/rmse_test_chain_" + str(self.temperatures[i]) + ".txt"
            )
            dat = np.loadtxt(file_name)
            rmse_test[i, :] = dat[burnin:]

            file_name = (
                self.path + "/predictions/rmse_train_chain_" + str(self.temperatures[i]) + ".txt"
            )
            dat = np.loadtxt(file_name)
        ...
        chain1_rmsetest = rmse_test[
            0, :
        ]  # to get posterior of chain 0 only (PT chain with temp 1)
...
def main():
    ...
    for i in range(5, 6):
        ...
        swap_interval = int(
            swap_ratio * NumSample / num_chains
        ...
        problemfolder = "/mnt/c/Users/George/Desktop/UNSW/honours/PT_EvalSwapRW/"  # change this to your directory for results output - produces large datasets
